...
def __checkBarCode(barCode):
    ...
    if not barCode.startswith(PARAM["RSDB5CodeFirst"]):return False
    return True
...
def T_01_scanCode_A(product):
    ...
    while not __checkBarCode(barCode):
        barCode = askForSomething(u"扫描条码", u"RSDB5单板条码扫描错误，请重新扫描",autoCommit=False)
...
def T_02_mainPowerTest_M(product):
    u"电源网络短路测试-RSDB5单板主输入电源及各分支电源网络是否短路测试"
    # powerList = ['主电源24V输入', 'TP1: 5V', 'TP2: 6V', 'TP3: 1.2V', 'TP4: 3.6V', 'TP5: 1.0V', 'TP7: 1.8V', 'TP9: 3.3V']
    # alist = []
    ...
    powerStr = "主电源24V输入, TP1: 5V, TP2: 6V, TP3: 1.2V, TP4: 3.6V, TP5: 1.0V, TP7: 1.8V, TP9: 3.3V"
...
def T_03_branchPowerTest_M(product):
    u"电源网络电压精度测试-RSDB5单板上电后各电源网络电压精度测试"
    time.sleep(1)
    manulCheck(u'提示', u'请将RSDB5单板上电，并用万用表测量各分支电路电压精度，点ESC继续', check="nothing")
    # powerList = ['TP1: 5V', 'TP2: 6V', 'TP3: 1.2V', 'TP4: 3.6V', 'TP5: 1.0V', 'TP7: 1.8V', 'TP9: 3.3V']
    # alist = []
    # for i in powerList:
    ...
    powerStr = "TP1: 5V, TP2: 6V, TP3: 1.2V, TP4: 3.6V, TP5: 1.0V, TP7: 1.8V, TP9: 3.3V"
