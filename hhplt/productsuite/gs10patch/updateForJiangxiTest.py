...
def T_03_updateInfoFields_A(product):
    u'''更新信息区数据-更新信息区数据'''
    sc = __askForPlateDeviceCom()
    readResult = sc.read_INFO(os.path.dirname(os.path.abspath(__file__))+os.sep+"versions\\"+SESSION["versionFileName"],0x1900)
    print "old config-rf-para:"+"".join(["%.2x"%c for c in readResult])
    readResult[11] = 0x07
    CONFIG_RF_PARA = readResult[:12]
    uiLog(u"开始写入CONFIG_RF_PARA，值:%s"% "".join(["%.2x"%c for c in CONFIG_RF_PARA]))
    sc.save_CONFIG_RF_PARA(CONFIG_RF_PARA)
    sc.finishBslWritten()
