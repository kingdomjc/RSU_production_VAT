...
from hhplt.productsuite.gs11.board_digital import __askForPlateDeviceCom
...
def T_03_receiveSensitivity_A(product):
    ...
    frame_num = SP('gs10.receiveSensitivity.frameNum',500,int)   #发送帧总数
    frame_scope_high = SP('gs10.receiveSensitivity.frameBorder',485,int) #帧数阈值
    
#    power_db = SP('gs11.receiveSensitivity.power',-30)
    #GS11的上述参数不使用服务端获得，而从本地配置文件中搞
...
def T_04_transmittingPower_A(product):
    u'''发射功率测试-判断发射功率'''
#    power_border_low = SP('gs11.sendPower.low',-5)
#    power_border_high = SP('gs11.sendPower.high',5)
