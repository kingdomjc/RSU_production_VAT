...
from hhplt.productsuite.gs10 import overall_unit,trading
...
def __checkObuTestStatus(laserCode):
    ...
    with ServerBusiness(testflow = True) as sb:
        idCode = sb.getProductIdByBindingCode(productName="GS10 OBU",codeName=u"镭雕条码",code=laserCode)
        ...
        if u'''MAC验证与重新交易测试项''' not in status["suiteStatus"]:
            for suite in (overall_unit,trading):
                sn = suite.suiteName;
