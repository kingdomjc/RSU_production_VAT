...
if __name__ == "__main__":
    ...
    if de_data == input_data:
        print_log("ecb check pass")
    else:
        print_error("ecb check fail")
    ...
    print_data(de_data, "de_data:")
    if de_data == input_data:
        print_log("cbc check pass")
    else:
        print_error("cbc check fail")

    #file test
    file_path = r"test2k.txt"
    ...
    en_data_str = "".join([chr(x) for x in en_data])
    f = open(ecb_path_en, 'w+b')
    f.write(en_data_str)
    f.close()
    ...
    f = open(ecb_path_de, 'w+b')
    f.write(de_data_str)
    f.close()
    if de_data == file_data_list:
        print_log("file decode pass")
    else:
        print_error("file decode fail")

    # 2. CBC
    sm4_d.sm4_set_key(key_data, ENCRYPT)
    en_data = sm4_d.sm4_crypt_cbc(iv_data, file_data_list)
    en_data_str = "".join([chr(x) for x in en_data])
    f = open(cbc_path_en, 'w+b')
    f.write(en_data_str)
    f.close()
