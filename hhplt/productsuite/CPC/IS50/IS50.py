...
class IS50Exception(Exception):

    def __init__(self, err_str):
        super(IS50Exception, self).__init__()
...
class IS50(object):
    ...
    def open(self, mode=0, dev="COM4"):
        ...
        if self.fd < 0:
            raise IS50Exception("open device return " + str(self.fd))
    ...
    def rsuInit(self, time, bstInterval, retryInterval, txPower, channelId, timeout):
        ...
        if ret != 0:
            raise IS50Exception("RSU_INIT_rq error! " + str(ret)) 
        ret = {}
    ...
    def resetPsam(self, sockId):
        ...
        if ret != 0:
            raise IS50Exception("resetPsam error! " + str(ret))
    ...
    def inventObu(self):
        ...
        if ret != 0:
            raise IS50Exception("Invent Obu Fail! " + str(ret))
        ret = {}
    ...
    def getSecure(self):
        ...
        if ret != 0:
            raise IS50Exception("getSecure fail! " + str(ret))
        if status.value != 0:
            raise IS50Exception("getSecure status not 0 " + str(status.value))
        ret = {}
        ret['did'] = did.value
        ret['fid'] = fid.value
    ...
    def transferChannel(self, channelId,  apduList, apdu):
        ...
        if ret != 0:
             raise IS50Exception("send transfer_channel fail %d " % ret)
        did = c_int(0)
        c_channelId = c_int(0)
        c_apduSize = c_int(0)
        c_status = c_int(0)
        ...
        if(ret != 0):
            raise IS50Exception("receive transfer_channel response fail %d " % ret)
        if(c_status.value != 0):
            raise IS50Exception("receive transfer_channel status error %d " % c_status.value)
    ...
    def setMMI(self):
        ...
        if ret != 0:
             raise IS50Exception("send setmmi fail %d " % ret)
        did = c_int(0)
        status = c_int(0)
        ret = self.dll.SetMMI_rs(self.fd, byref(did), byref(status), 500)
        if(ret != 0):
            raise IS50Exception("receive setmmi fail %d " % ret)
        if(status.value != 0):
            raise IS50Exception("receive setmmi status error %d " % status.value)

    def eventReport(self):
        ret = self.dll.EVENT_REPORT_rq(self.fd, 0, 0, 0, 500)
        if(ret != 0):
            raise IS50Exception("send eventReport fail %d " % ret)
    ...
    def hfInvent(self):
        ...
        if(ret != 0):
            raise IS50Exception("recv hf card inventory response " + str(ret))
        if status.value != 0:
            raise IS50Exception("recv hf card inventory response status not zero " + str(status.value))
    ...
    def hfExchangeApdu(self, apduList, apdu):
        ...
        if(ret != 0):
            raise IS50Exception("recv cpu card  response " + str(ret))
    ...
    def MIreadCardId(self):
        ...
        if ret != 0:
            raise IS50Exception("MI read CardId fail" + str(ret))
    ...
    def MISetEnterMode(self, cpcid):
        ...
        if ret != 0:
            raise IS50Exception("MISetEnterMode fail" + str(ret))
    ...
    def MISetExitMode(self, cpcid):
        ...
        if ret != 0:
            raise IS50Exception("MISetExitMode fail" + str(ret))
        ret = {}
        (p, ) = struct.unpack("H", self.buffer.raw[0:2])
        power = 579*3.3/(p-10)
        ret["power"] = power 
    ...
    def MIShowCardStatus(self, cpcid):
        ...
        if ret != 0:
            raise IS50Exception("MIShowCardStatus fail" + str(ret))
    ...
    def MIGetCardData(self, cpcid):
        ...
        if ret != 0:
            raise IS50Exception("MIGetCardData fail" + str(ret))
