...
from IS50.SAPI import *
from IS50.crypto import *
...
def str_to_hex(str_to_switch):	
	init_datahex = unhexlify(str_to_switch)
	get_hex = []
...
def cpc_card_update():
    ...
    if ret != '':
        raise StandardError("system_info update failed")
    print "\n*** update sysinfo completed ***\n"
    ...
    new_master_key = PARAM["masterKey"]
    ret = update_master_key(new_master_key, DE_DELIVERY) # do not delivery
    if ret != '':
        raise StandardError("master_key update failed")
    print "\n*** update master key of MF completed ***\n"
    ...
    if ret:
        ...
        print "enter DF01 failed\n"
        raise StandardError("enter DF01 failed")


    enDeliveryType = PARAM["distriLevel"]   #分散级数1或2，取代原来的 EN_DELIVERY_L1 和 EN_DELIVERY_L2

	#***** update internal authentication key 1 **********
	#inter_auth_key_1 = defaultkey
    #inter_auth_key_1 = key_all_zero
    inter_auth_key_1 = PARAM["ik1"]
    inter_auth_key_1_attr = '020100'
    ret = update_other_key(inter_auth_key_1_attr, inter_auth_key_1, enDeliveryType)
    if ret != '':
        raise StandardError("inter_auth_key_1 update failed")
    print "\n*** update inter_auth_key_1 completed ***\n"

	
	#***** update internal authentication key 2 **********
    inter_auth_key_2 = PARAM["ik1"]
    #inter_auth_key_2 = defaultkey
    inter_auth_key_2_attr = '020200'	
    ret = update_other_key(inter_auth_key_2_attr, inter_auth_key_2, enDeliveryType)
    if ret != '':
        raise StandardError("inter_auth_key_2 update failed")
    print "\n*** update inter_auth_key_2 completed ***\n"


	#***** update external authentication key 1 **********
    #exter_auth_key_1 = init_defaultkey
    exter_auth_key_1 = PARAM["uk1"]
    exter_auth_key_1_attr = '000100'
    ret = update_other_key(exter_auth_key_1_attr, exter_auth_key_1, enDeliveryType)
    if ret != '':
        raise StandardError("exter_auth_key_1 update failed")
    print "\n*** update exter_auth_key_1 completed ***\n"


	#***** update external authentication key 2 **********
    #exter_auth_key_2 = init_defaultkey
    exter_auth_key_2 = PARAM["uk2"]
    exter_auth_key_2_attr = '000200'
    ret = update_other_key(exter_auth_key_2_attr, exter_auth_key_2, enDeliveryType)
    if ret != '':
        raise StandardError("exter_auth_key_2 update failed")
    print "\n*** update exter_auth_key_2 completed ***\n"


	#***** update external authentication key 3 ********** #### ret 6A 88: No key data
    exter_auth_key_3 = PARAM["uk1"]
    exter_auth_key_3_attr = '000300'
    ret = update_other_key(exter_auth_key_3_attr, exter_auth_key_3, enDeliveryType)
    if ret != '':
        raise StandardError("exter_auth_key_3 update failed")
    print "\n*** update exter_auth_key_3 completed ***\n"


	#***** update external authentication key 4 ********** #### ret 6A 88: No key data
    exter_auth_key_4 = PARAM["uk2"]
    exter_auth_key_4_attr = '000400'
    ret = update_other_key(exter_auth_key_4_attr, exter_auth_key_4, enDeliveryType)
    if ret != '':
        raise StandardError("exter_auth_key_4 update failed")
    print "\n*** update exter_auth_key_4 completed ***\n"
		

	#************** update DAMK_DF01 key *************	
    DAMK_DF01_str = PARAM["masterKey"]
	#DAMK_DF01_str = defaultkey
	#*** update DAMK_DF01
    ret = update_other_key('010100', DAMK_DF01_str, DE_DELIVERY) # do not delivery
    if ret != '':
        raise StandardError("DAMK of DF01 update failed")
    ...
    ret = update_master_key(new_master_key, DE_DELIVERY) # do not delivery
    if ret != '':
        raise StandardError("master key of DF01 update failed")
...
def cpc_esam_recreate():
   ...
   #******************* # erease file ************************************* ####
    ...
    if ret != '':
        raise StandardError("HF_CMD run failed")
    ...
    ret = update_other_key('010100', DAMK_MF_str, DE_DELIVERY) # do not delivery
    if ret != '':
        raise StandardError("DAMK_MF load failed")
    ...
    ret = update_other_key('010100', DAMK_DF01_str, DE_DELIVERY) # do not delivery
    if ret != '':
        raise StandardError("DAMK of DF01 load failed")

	#************************** load MK_DF01 ******************************* ####
    new_master_key = defaultkey
    ret = update_master_key(new_master_key, DE_DELIVERY) # do not delivery
    if ret != '':
        raise StandardError("master key of DF01 load failed")

	#************************** Create file_DF01 ***************************** ####
	# create EF01_DF01
    HFCARD_COMMAND('80E0EF0107080040F011FFFF') # 80E0EF010708 0040 F011FFFF
	# create EF02_DF01
	...
	# create EF06_DF01
    HFCARD_COMMAND('80E0EF0607080080F0F0FFFF') # 80E0EF0607080080F0F0FFFF
	# create EF07_DF01
...
def cpc_update_startup(input_defaultkey, delivery_or_not, input_issue_info, input_cpc_id = ''):
	...
	# get_sysinfo = 'B9F3D6DD520100015201ACFF00000001' + 'FFFFFFFFFFFFFFFFFFFFFFFFFFFF'  #********************** default sysinfo
