...
class CpcTraderIssuer(TestResource):
    ...
    def initResource(self):
        ret = self.reader.RSU_Open(2,None,None)
    ...
    def cpcReadMac(self):
        # 射频方式读取MAC，在ESAM的DF01目录下的EF06文件里
        for apdu in ("00A4000002DF01","00B0860008"):
            ret, APDULIST, Data, ReturnStatus = self.reader.HFCmd(1,"%.2x%s"%(len(apdu)/2, apdu))
            Data = Data[2:int(Data[:2],16)*2 + 2]
            if ret != 0 or ReturnStatus != 0 or Data[-4:] != "9000":
                raise TestItemFailException(failWeight=10,message=u"射频方式读取ESAM里的MAC失败")
        return Data[8:16]

    def cpcUpdateMacInEsam(self,targetMac):
        # 射频方式写MAC到ESAM里面
        for apdu in ("00A4000002DF01","00D686000866666666%s"%targetMac):
            ret, APDULIST, Data, ReturnStatus = self.reader.HFCmd(1,"%.2x%s"%(len(apdu)/2, apdu))
            Data = Data[2:int(Data[:2],16)*2 + 2]
            if ret != 0 or ReturnStatus != 0 or Data[-4:] != "9000":
                raise TestItemFailException(failWeight=10,message=u"射频写ESAM里的MAC失败")
    ...
    def cpcHFEnterMF(self):
        APDU = "0700A40000023F00"
        self.__record('cpcHFEnterMF.rq', u'send')
        ret, APDULIST, Data, ReturnStatus = self.reader.HFCmd(1, APDU)
        uiLog("cpcHFEnterMF.rp: ret=%dAPDULIST=%d,Data=%s,ReturnStatus=%d" % (ret, APDULIST, Data, ReturnStatus))
        if ret != 0 or ReturnStatus != 0:
            self.__record('cpcHFEnterMF.rp', u'接收异常 error code = %d' % (ret))
            raise TestItemFailException(failWeight=10,
                                        message=u'cpcHFReadBaseInfo.rp,ret =%d,ReturnStatus=%d' % (ret, ReturnStatus))
        self.__record('cpcHFEnterMF.rp', u'success')

    def cpcHFReadBaseInfo(self):
        #self.reader.HFActive(200)
        # 读取CPC状态
        APDU = "0500b0820020"
    ...
    def deliverKey(self,psamSoftId,cpcId,deliverType = "entryExit"):
        # 分散密钥
        # 捷德测试PSAM 28 41
        # 交通部提供捷德SM4 PSAM
        lc = "10"
        p1p2 = "4844" if deliverType == "entryExit" else "4845"
        APDU = "15"+"801A" + p1p2 + lc + cpcId + GLOBAL["cpcarea"] + GLOBAL["cpcarea"]
        # 我们自己的测试卡
        # lc = "08"
        # p1p2 = "2841"
        # APDU = "0d" + "801A" + p1p2 + lc + cpcId
        uiLog(u"delverkey:" + APDU)
    ...
    def cpcHFexternalAuth(self,cpc_mac):
        ...
        APDU = "0082000108" + arthDatastr
        APDU = "%.2x%s" % (len(APDU) / 2, APDU)
        uiLog("cpcHFexternalAuth:" + APDU)
        ...
        if Data[2:][apduLength * 2 - 4:apduLength * 2] == "6983":
            self.__record('cpcHFexternalAuth.rp', u'error code =%d' % (int(Data[:2], 16)))
            raise TestItemFailException(failWeight = 10,message = u'认证失败，已锁卡:cpcHFexternalAuth,error code=%s' % Data[-4:])
        if ret != 0:
    ...
    def cpcHFWriteEntryInfo(self,cardState):
        ...
        uiLog(u"cpcHFWriteEntryInfo:" + APDU)
        self.__record('cpcHFWriteEntryInfo.rq', u'send')
        ret, APDULIST, Data, ReturnStatus = self.reader.HFCmd(1, APDU)
        uiLog(u"cpcHFWriteEntryInfo.rp: ret=%dAPDULIST=%d,Data=%s,ReturnStatus=%d" %  (ret, APDULIST, Data, ReturnStatus))
        if ret != 0 or ReturnStatus != 0:
    ...
    def cpcHFClearRouteInfo(self, routeIndex):
        ...
        cmdLen = "65"
        cmdFileLen = "60"
        cmdFile = "0000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000000000000000000000000000000"

        # cmdLen = "0C"
        # cmdFileLen = "07"
        # cmdFile = "0000FFFFFFFFFF"
    ...
    def integrateCpcSignRoute(self,psamSlotId,cpcId):
        ...
        self.cpc_bst_aid1(1)
        self.deliverKey(psamSlotId,cpcId,deliverType = "5.8")
    ...
    def cpc_write_route_file_authen(self):
        ...
        if ret != 0 or ReturnStatus != 0:
            ...
            raise Exception(u"cpc_write_route_file.rp ReturnStatus not 0")
