...
def setup(product):
    ENTRY.setup(product,CpcTraderIssuer)
...
def T_05_routeSign_A(product):
    ...
    time.sleep(2)
    try:
        testRouteInfo = rsu.integrateCpcSignRoute(PARAM["psamSlotId"],product.param["cpcId"])
        uiLog(u"测试模拟路径：%s"%testRouteInfo)
        product.param["testRouteInfo"] = testRouteInfo
    except Exception,e:
        raise TestItemFailException(failWeight=10,message=str(e))
