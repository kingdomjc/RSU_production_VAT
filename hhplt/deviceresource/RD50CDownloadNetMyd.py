...
from hhplt.deviceresource.RD50CNetSendRecvMyd import PlateDevice as BslDevice, MODE_SEND_SMALL_POWER
...
class GS10PlateDevice(TestResource):
    ...
    def downloadVersion(self, version_file, password_file=None):
        ...
        try:
            ...
            if password_file is None:
                downlv = device.download_ver(None, version_file, True)
                if downlv == "OK":
                    return "OK"
                elif downlv == "loginfail":
                    return "loginfail"
                elif downlv == "TFTPfail":
                    return "TFTPfail"
                elif downlv == "EPLDError":
                    return "EPLDError"
                elif downlv == "unknowError":
                    return "unknowError"
            else:
    ...
    def downloadVersion2(self, version_file, password_file=None):
        '''下载版本，入参是密码文件、版本文件的路径'''
        device = self.bslDevice
        try:
            # 如果不带密码，就擦一下；带密码，就用密码
            if password_file is None:
                downlv = device.download_ver2(None, version_file, True)
                if downlv == "OK":
                    return "OK"
                elif downlv == "loginfail":
                    return "loginfail"
