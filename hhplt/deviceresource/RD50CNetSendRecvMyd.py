...
#         self.serial.setTimeout(5)
    ...
    def recvReboot(self):
        ...
        while True:
            ...
            if "turn off boot console udbg0" in response:
                return "ok"

    def download_ver2(self, old_file_path=None, new_file_path=None, mass_erase=False):
        loginResult = self.register_and_serial("shhtj", "4. reset")
        if loginResult == "loginfail":
            return "loginfail"
        elif loginResult == "EPLDError":
            return "EPLDError"
        self.serial.write("setenv ipaddr %s\r" % str(PARAM["defaultNetOneIp"]))
        self.serial.write("setenv serverip %s\r" % str(PARAM["localIP"]))
        downResult = self.forward_and_check("download\r","=>")
        if downResult == "TFTPfail":
            return "TFTPfail"
        elif downResult == "unknowError":
            return "unknowRrror"
        self.forward_and_check("reset\r","turn off boot console udbg0")
        return "OK"
    ...
    def download_ver(self, old_file_path=None, new_file_path=None, mass_erase=False):
        ...
        self.serial.write("setenv ipaddr %s\r" % str(PARAM["defaultNetOneIp"]))
        self.serial.write("setenv serverip %s\r" % str(PARAM["localIP"]))
        self.serial.write("setenv ethact UEC1\r")
        downResult = self.forward_and_check("download\r","=>")
        if downResult == "TFTPfail":
            return "TFTPfail"
        elif downResult == "unknowError":
