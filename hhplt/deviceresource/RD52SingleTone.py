...
class DeviceProxy(object):
    ...
    def open(self, ipaddr):
        try:
            ...
            raise TestResourceInitException(u"请检查测试工装网络连接是否正常")
...
class FpgaPll(object):
    ...
    def config(self):
        ...
        proxy._write_pll_reg(0, pll_reg_map)
        proxy._write_pll_reg(16, pll_reg_map1)
        proxy._write_pll_reg(32, pll_reg_map2)
    ...
    def config1(self):
        ...
        pll_reg_map2 = [0x00, 0x00, 0x00, 0x00, 0x6d, 0x02, 0x05,
                        0x05, 0xf3, 0xcc, 0x33, 0xcb, 0x00, 0x40, 0x02]
        proxyFrock._write_pll_reg(0, pll_reg_map)
        proxyFrock._write_pll_reg(16, pll_reg_map1)
        proxyFrock._write_pll_reg(32, pll_reg_map2)


class TxPll(object):

    def _write_and_trig(self, value):
        proxy._write_fpga_reg(0x54, value)
        proxy._write_fpga_reg(0x55, 0x0000)
        proxy._write_fpga_reg(0x55, 0x0001)
        proxy._write_fpga_reg(0x55, 0x0000)

    def _write_and_trig1(self, value):
        proxyFrock._write_fpga_reg(0x54, value)
        proxyFrock._write_fpga_reg(0x55, 0x0000)
        proxyFrock._write_fpga_reg(0x55, 0x0001)
        proxyFrock._write_fpga_reg(0x55, 0x0000)
    ...
    def config1(self, channel):
        if channel == 5830:
            print "channel freq is %d " % 5830
            self._write_and_trig1(0x0204)
            self._write_and_trig1(0x0404)
            # self._write_and_trig(0x063e)
            self._write_and_trig1(0x1030)
            self._write_and_trig1(0x084d)
            self._write_and_trig1(0x0a11)
            self._write_and_trig1(0x0c08)
            self._write_and_trig1(0x0e91)
            # self._write_and_trig(0x1030)
            self._write_and_trig1(0x063e)
            self._write_and_trig1(0x1200)
            self._write_and_trig1(0x1400)
            self._write_and_trig1(0x16b9)
            self._write_and_trig1(0x180d)
            self._write_and_trig1(0x1ac0)
        elif channel == 5790:
            print "channel freq is %d" % 5790
            self._write_and_trig1(0x0204)
            self._write_and_trig1(0x0404)
            self._write_and_trig1(0x063e)
            self._write_and_trig1(0x084d)
    ...
    def is_lock(self):
        if proxy._read_fpga_reg(0x34) == 1:
            return True
        else:
            return False
...
class RxPll(object):
    ...
    def config1(self, channel):
        if channel == 5762:
            ...
            self._write_and_trig1(0x0c08)
            self._write_and_trig1(0x0e90)
            self._write_and_trig1(0x1003)
            self._write_and_trig1(0x1233)
            self._write_and_trig1(0x1430)
            self._write_and_trig1(0x16b9)
            self._write_and_trig1(0x180d)
            self._write_and_trig1(0x1ac0)
    ...
    def is_lock(self):
        if proxy._read_fpga_reg(0xf4) == 1:
            return True
        else:
            return False
...
class B_Adc(object):
    ...
    def _write_and_trig(self, value):
        proxy._write_fpga_reg(0xa3, value)
        proxy._write_fpga_reg(0xa2, 0)
        proxy._write_fpga_reg(0xa2, 1)
        proxy._write_fpga_reg(0xa2, 0)

    def _adjust(self):
        proxy._write_fpga_reg(0xa0, 0x0000)
        proxy._write_fpga_reg(0xa0, 0x0001)

    def select_channel(self, value):
        proxy._write_fpga_reg(0xa1, value)
    ...
    def training(self):
        ...
        value = proxy._read_fpga_reg(0x9c)
        print hex(value)
        cnt = 0
...
class MacCtrl(object):
    ...
    def send_test_frame(self, num, flag):
        proxy._write_fpga_reg(0x20, 0x0003)  # FORWARD_FILT_EN
        proxy._write_fpga_reg(0x21, 0x7ffe)  # FORWARD_PWADJ
        proxy._write_fpga_reg(0x22, 0x47e)  # ask_high_val max:1f00
        proxy._write_fpga_reg(0x23, 0xa28)  # ask_low_val
        proxy._write_fpga_reg(0x24, 0xa28)  # ask_no_val
        proxy._write_fpga_reg(0x0e, 0x0003)  # wake signal guard 0.5ms
        proxy._write_fpga_reg(0x44, 0x0025)  # resend interval 5ms
        proxy._write_fpga_reg(0x56, 0x003c)
        proxy._write_fpga_reg(0x57, 0x0000)
        proxy._write_fpga_reg(0x57, 0x0001)
        proxy._write_fpga_reg(0x57, 0x0000)
        proxy._write_fpga_reg(0x45, num)  # resend cnt
        proxy._write_fpga_reg(0x46, 0x0000)  #
        '''
        ...
        time.sleep(0.1)
        if 1 == flag:
            proxy._write_fpga_reg(0x09, 0x0003)
            proxy._write_fpga_reg(0x23, 0x7d0)  # ask_low_val
            proxy._write_fpga_reg(0x24, 0x7d0)  # ask_no_val
            proxy._write_fpga_reg(0x0e, 0x0003)  # wake signal guard 0.5ms
            proxy._write_fpga_reg(0x44, 0x003f)  # resend interval 5ms
            proxy._write_fpga_reg(0x56, 0x003c)
            proxy._write_fpga_reg(0x57, 0x0000)
            proxy._write_fpga_reg(0x57, 0x0001)
            proxy._write_fpga_reg(0x57, 0x0000)
...
if __name__ == '__main__':
    proxy = DeviceProxy()
    proxy.init_fpga()
    proxy.read_fpga_reg_pll(0x0001)
