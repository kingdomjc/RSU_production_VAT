...
class SerialBSL5(bsl5.BSL5):
    ...
    def bsl(self, cmd, message='', expect=None):
        ...
        #print str(message)
        self.logger.debug('Command 0x%02x %s' % (cmd, str(message).encode('hex')))
        # prepare command with checksum
        txdata = struct.pack('<BHB', 0x80, 1+len(message), cmd) + message
        txdata += struct.pack('<H', reduce(crc_update, bytearray(txdata[3:]), 0xffff))   # append checksum
        ...
        if pi == 0x80:
            ...
            crc_expected = reduce(crc_update, bytearray(data), 0xffff)
    ...
    def start_bsl(self):
        ...
        self.set_TEST(True)
        self.set_RST(False)      # power supply        
        #time.sleep(0.125)
        self.set_TEST(False)
        time.sleep(0.3)
        self.set_TEST(True)
